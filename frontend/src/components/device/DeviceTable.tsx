              ...
              <tr key={d.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <td className="p-3">
                  {imageUrl ? (
                    <button
                      ...
                      className="relative group rounded border border-slate-300 dark:border-slate-700 overflow-hidden block w-10 h-10 bg-slate-100 dark:bg-slate-950 hover:border-cyan-500 transition-colors"
                    ...
                    <div className="flex items-center justify-center w-10 h-10 rounded bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800/40 text-indigo-600 dark:text-indigo-400">
                ...
                <td className="p-3 font-medium text-slate-900 dark:text-slate-100">
                  ...
                  {isNvd && (
                    <button
                      ...
                      className="mt-1 inline-flex items-center gap-1 text-xs text-indigo-600 dark:text-indigo-400 hover:underline font-normal"
                    >
                  ...
                  {isPrinter ? (
                    <div className="flex flex-wrap items-center gap-1 mt-1 font-normal">
                      ...
                      <span className="rounded bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 text-[10px] text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                ...
                <td className="p-3">
                  <span
                    ...
                    style={{
                      backgroundColor: `${d.type_color || '#64748b'}22`,
                      color: d.type_color,
                    }}
                  >
                    {d.type_name}
                ...
                <td className="p-3 font-mono text-xs text-slate-700 dark:text-slate-300">{d.ip_address || '—'}</td>
                <td className="p-3 font-mono text-xs text-slate-700 dark:text-slate-300">{d.mac_address || '—'}</td>
                <td className="p-3 text-slate-700 dark:text-slate-300">
                  <div>{d.location || '—'}</div>
                  {meta.nvd_device_name && (
                    <span className="inline-block mt-0.5 rounded bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800/30 px-1.5 py-0.5 text-[10px] text-indigo-700 dark:text-indigo-300">
